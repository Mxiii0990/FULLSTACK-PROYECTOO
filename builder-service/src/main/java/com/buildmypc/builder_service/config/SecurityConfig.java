package com.buildmypc.builder_service.config;
...
// Seguridad del microservicio de builder.
//
...
//
// Es la segunda barrera de seguridad: aunque alguien evite el gateway y llame directo al puerto de este msvc,
...
public class SecurityConfig {
    ...
    // El token guarda los roles en un claim llamado "roles" (ej: ["ROLE_TECNICO"]).
    ...
    public SecurityFilterChain filterChain(HttpSecurity http, JwtAuthenticationConverter converter) throws Exception {
        http
                ...
                // request) no aplica, así que se desactiva para no bloquear los POST/PUT/DELETE.
                .csrf(csrf -> csrf.disable())
                // Las reglas se evalúan EN ORDEN: la primera que coincide con la petición manda.
                .authorizeHttpRequests(auth -> auth
                        // Documentación y consola h2: abiertas (sin token) para poder mostrarlas sin problemas.
                        ...
                        .requestMatchers(HttpMethod.GET, "/api/builds/**", "/api/v2/builds/**")
                        .hasAnyRole("ADMIN", "TECNICO", "CLIENTE")
                        // ESCRIBIR (todo lo que no sea GET sobre esas rutas: POST/PUT/DELETE): solo ADMIN o TECNICO.
                        .requestMatchers("/api/builds/**", "/api/v2/builds/**")
